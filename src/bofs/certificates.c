/*
 * certificates.c — BOF for DPAPI certificate triage
 *
 * Usage:
        ...
        if (target) {
            ...
            if (attrs != INVALID_FILE_ATTRIBUTES) {
                if (attrs & FILE_ATTRIBUTE_DIRECTORY)
                    triage_cert_folder(&cache, target, (BOOL)show_all);
                else
                    triage_cert_file(&cache, target, (BOOL)show_all);
            }
        } else {
            triage_user_certs(&cache, NULL, server, (BOOL)show_all);
        }
    }
