/*
 * rdg.c — BOF for Remote Desktop credential triage
 *
 * Usage:
    ...
    if (target) {
        ...
        if (attrs != INVALID_FILE_ATTRIBUTES) {
            if (attrs & FILE_ATTRIBUTE_DIRECTORY)
                triage_rdg_folder(&cache, target, (BOOL)unprotect);
            else
                triage_rdg_file(&cache, target, (BOOL)unprotect);
        }
    } else {
        /* Full RDCMan triage across user profiles */
        triage_rdcman(&cache, NULL, (BOOL)unprotect);
    }
